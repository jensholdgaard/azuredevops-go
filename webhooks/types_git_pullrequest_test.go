...
func TestGitPullRequestMergedEvent_RealPayload_StrictDecode(t *testing.T) {
	data, err := os.ReadFile("testdata/git.pullrequest.merged.real.json")
	if err != nil {
		t.Fatalf("read fixture: %v", err)
	}
	...
	if event.Resource.ClosedDate != "2014-06-30T18:59:12.3660573Z" {
		t.Errorf("closedDate = %q, want %q", event.Resource.ClosedDate, "2014-06-30T18:59:12.3660573Z")
	}
	if event.Resource.CreatedBy.DisplayName != "Jamal Hartnett" {
		t.Errorf("createdBy.displayName = %q, want %q", event.Resource.CreatedBy.DisplayName, "Jamal Hartnett")
	}
